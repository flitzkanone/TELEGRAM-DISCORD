...
DISCORD_IDS_MSG_HEADER = "--- BOT DISCORD IDS (DO NOT DELETE) ---" # NEU
...
class DiscordWebhookManager:
    ...
    def execute_webhook_action(webhook_url: str, message_id: str = None, embed: DiscordEmbed = None, content: str = None, action: str = 'send'):
        ...
        try:
            if action == 'edit' and message_id:
                webhook = DiscordWebhook(url=f"{webhook_url}/messages/{message_id}")
            else:
            ...
            response = webhook.edit() if action == 'edit' else webhook.execute(remove_embeds=True)
    ...
    def create_stats_dashboard_embed(stats: dict) -> DiscordEmbed:
        ...
        active_users_24h = 0; now = datetime.now()
        ...
        embed = DiscordEmbed(title="📊 Bot-Statistik Dashboard (Live)", color="2ecc71")
        ...
        embed.add_embed_field(name="🖱️ Klick-Verhalten", value=f"Vorschauen: **{events.get('next_preview', 0)}**\nPaketauswahl: **{events.get('package_selected', 0)}**")
        return embed
...
def load_stats():
    try:
        ...
        return {
            ...
            "discord_persistence_message_id": None, # NEU
            "users": {}, 
            "admin_logs": {}, 
            "events": {},
            "discord_message_ids": {} # NEU
        }
...
# --- NEU: Discord ID Persistenz ---
async def save_discord_ids_to_telegram(context: ContextTypes.DEFAULT_TYPE):
    """Speichert die Discord-Nachrichten-IDs in einer separaten Telegram-Nachricht."""
    if not NOTIFICATION_GROUP_ID: return
    stats = load_stats()
    discord_ids_to_save = stats.get("discord_message_ids", {})
    if not discord_ids_to_save: return

    json_string = json.dumps(discord_ids_to_save, indent=2)
    message_text = f"{DISCORD_IDS_MSG_HEADER}\n<tg-spoiler>{json_string}</tg-spoiler>"
    discord_message_id = stats.get("discord_persistence_message_id")

    try:
        if discord_message_id:
            await context.bot.edit_message_text(chat_id=NOTIFICATION_GROUP_ID, message_id=discord_message_id, text=message_text, parse_mode='HTML')
        else:
            raise error.BadRequest("No Discord ID message found")
    except error.BadRequest:
        logger.warning("Discord ID persistence message not found, creating a new one.")
        try:
            sent_message = await context.bot.send_message(chat_id=NOTIFICATION_GROUP_ID, text=message_text, parse_mode='HTML')
            stats["discord_persistence_message_id"] = sent_message.message_id
            save_stats(stats)
        except Exception as e:
            logger.error(f"Could not create a new Discord ID persistence message: {e}")

async def load_discord_ids_from_telegram(application: Application):
    """Lädt die Discord-Nachrichten-IDs aus der Telegram-Nachricht beim Bot-Start."""
    if not NOTIFICATION_GROUP_ID:
        logger.info("No notification group ID, skipping Discord ID restore.")
        return
    
    logger.info("Attempting to restore Discord message IDs from Telegram...")
    stats = load_stats()
    discord_message_id = stats.get("discord_persistence_message_id")

    if not discord_message_id:
        logger.warning("No Discord persistence message ID in stats.json. Cannot restore IDs.")
        return

    try:
        message = await application.bot.get_message(chat_id=NOTIFICATION_GROUP_ID, message_id=discord_message_id)
        json_match = re.search(r'<tg-spoiler>(.*)</tg-spoiler>', message.text_html, re.DOTALL)
        if not json_match:
            logger.error("Could not find spoiler tag in Discord ID message.")
            return

        discord_ids_data = json.loads(json_match.group(1))
        stats["discord_message_ids"] = discord_ids_data
        save_stats(stats)
        logger.info(f"Successfully restored {len(discord_ids_data)} Discord message IDs.")
    except Exception as e:
        logger.error(f"An unexpected error occurred during Discord ID restore: {e}")
...
async def send_or_update_admin_log(context: ContextTypes.DEFAULT_TYPE, user: User, event_text: str = ""):
    ...
    if NOTIFICATION_GROUP_ID:
        admin_logs = stats.get("admin_logs", {}); log_message_id = admin_logs.get(user_id_str, {}).get("message_id")
    ...
    if DISCORD_USER_LOG_WEBHOOK_URL:
        ...
        action = 'edit' if discord_msg_id else 'send'
        
        response = DiscordWebhookManager.execute_webhook_action(DISCORD_USER_LOG_WEBHOOK_URL, message_id=discord_msg_id, embed=embed, action=action)
        
        if not discord_msg_id and response and 'id' in response:
            discord_ids[user_id_str] = response['id']
            stats["discord_message_ids"] = discord_ids
            save_stats(stats)
            await save_discord_ids_to_telegram(context)
...
async def update_pinned_summary(context: ContextTypes.DEFAULT_TYPE):
    ...
    if DISCORD_STATS_WEBHOOK_URL:
        ...
        action = 'edit' if dashboard_msg_id else 'send'

        response = DiscordWebhookManager.execute_webhook_action(DISCORD_STATS_WEBHOOK_URL, message_id=dashboard_msg_id, embed=embed, action=action)
        
        if not dashboard_msg_id and response and 'id' in response:
            discord_ids["stats_dashboard"] = response['id']
            stats["discord_message_ids"] = discord_ids
            save_stats(stats)
            await save_discord_ids_to_telegram(context)

# [ HIER FOLGT DER REST DEINES BOT-CODES OHNE ÄNDERUNGEN ]
# (Von async def restore_stats_from_pinned_message bis zum Ende)
...
async def send_preview_message(update: Update, context: ContextTypes.DEFAULT_TYPE, schwester_code: str):
    ...
    if schwester_code == 'gs': caption = f"Heyy ich bin Anna, ich bin <tg-spoiler>{AGE_ANNA} </tg-spoiler> Jahre alt und mache mit meiner Schwester zusammen 🌶️ videos und Bilder falls du lust hast speziele videos zu bekommen schreib mir 😏 @Anna16_brln"
    else: caption = f"Heyy, mein name ist Luna ich bin <tg-spoiler>{AGE_LUNA} </tg-spoiler> Jahre alt und mache 🌶️ videos und Bilder. wenn du Spezielle wünsche hast schreib meiner Schwester für mehr.\nMeine Schwester: @Anna16_brln"
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith(("pay_paypal:", "pay_voucher:", "pay_crypto:")):
        ...
        amount = int(amount_str); base_price = PRICES[media_type][amount]
        stats = load_stats(); user_data = stats.get("users", {}).get(str(user.id), {}); package_key = f"{media_type}_{amount}"
        price = get_discounted_price(base_price, user_data.get("discounts"), package_key)
        if price == -1: price = base_price
        if data.startswith("pay_paypal:"):
            await track_event("payment_paypal", context, user.id); await update_payment_log("PayPal (2 für 1)", price)
            if NOTIFICATION_GROUP_ID:
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("awaiting_voucher"):
        provider = context.user_data.pop("awaiting_voucher"); code = text_input; vouchers = load_vouchers()
        vouchers[provider].append(code); save_vouchers(vouchers)
...
async def post_init(application: Application):
    ...
    await load_discord_ids_from_telegram(application) # NEU
